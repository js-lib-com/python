...
def string(text):
    """Replace English contraction form with word components."""
    if isinstance(text, list):
        return [string(s) for s in text]
    pattern = re.compile(r'\b(' + '|'.join(contraction_mapping.keys()) + r')\b')
    expanded_text = pattern.sub(lambda match: contraction_mapping[match.group(0)], text)
    return expanded_text


# add apostrophe to custom filters; by default tokenizer does not filter out the apostrophe
tokenizer = Tokenizer(filters='!"\'#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n')

text = ["This is an example 'sentence' for tokenization.It wasn't plain-simple."]
tokenizer.fit_on_texts(string(text))
seq = tokenizer.texts_to_sequences(string(text))[0]
print(seq)
